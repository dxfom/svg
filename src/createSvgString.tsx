...
const defaultOptions: CreateSvgStringOptions = {
  warn: console.debug,
...
const entitiesToSvgString = (dxf: DxfReadonly, entities: DxfReadonly['ENTITIES'], options: CreateSvgStringOptions) => {
  const { warn } = options
...
export const createSvgString = (dxf: DxfReadonly, options?: Partial<CreateSvgStringOptions>) => {
  ...
  return (
    <svg
      ...
      viewBox={viewBox(dxf)}
    >
      {entitiesToSvgString(dxf, dxf.ENTITIES, resolvedOptions)}
    </svg>
  )
}
